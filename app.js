...
function bindConsultantPortal(){
  ...
  changePassForm?.addEventListener('submit', async (e)=>{
    ...
    cpMsg.textContent = '';
    if(!PASSWORD_CHANGE_URL){ cpMsg.textContent = 'Erro: URL de alteração de senha ausente.'; return; }
    ...
    if(!email || !senhaAtual || !novaSenha || !repetir){ cpMsg.textContent = 'Preencha todos os campos.'; return; }
    if(novaSenha !== repetir){ cpMsg.textContent = 'A nova senha e a confirmação não coincidem.'; return; }

    try{
      const senhaAtualHash = await sha256Hex(senhaAtual);
      const novaSenhaHash  = await sha256Hex(novaSenha);
      const resp = await fetch(PASSWORD_CHANGE_URL,{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ metadata:{ modo:'consultor_alterar_senha', enviadoEm:new Date().toISOString() }, alterar:{ email, senhaAtualHash, novaSenhaHash } }) });
      const data = await resp.json().catch(()=> ({}));
    ...
    }catch(err){ console.error(err); cpMsg.textContent = 'Erro de rede ao atualizar senha.'; }
  });
...
// ====== Logins rápidos da tela inicial
 (navegam para as telas internas) ======
