(function(){
  ...
  if(outroEmprego)
    outroEmprego.addEventListener('change', e =>
      cnpjWrapper.classList.toggle('hidden', e.target.value !== 'sim')
    );

  if(aposentado)
    aposentado.addEventListener('change', e =>
      aposentWrapper.classList.toggle('hidden', e.target.value !== 'sim')
    );

  if(temParente)
    temParente.addEventListener('change', e =>
      parentescoWrapper.classList.toggle('hidden', e.target.value !== 'sim')
    );
  ...
  form.addEventListener('submit', async (ev)=>{
    ...
    for(const f of files){
      if(f.size > 10*1024*1024){
        out.textContent=`O arquivo ${f.name} excede 10 MB.`;
        return;
      }
    ...
    if(canvas){
      assinaturaBase64 = canvas.toDataURL('image/png').split(',')[1];
    }
...
Por fim, fico ciente das responsabilidades pelas declarações prestada e firmo a presente.`,
        ...
        assinatura:{
          ...
          contentBase64:assinaturaBase64
        }
      }
    };
    ...
    try{
      ...
      const ok=resp.ok;
      out.textContent = ok ? 'Cadastro enviado com sucesso.' : 'Falha ao enviar.';
      if(ok) form.reset();
    }
    catch(e){
      console.error(e);
